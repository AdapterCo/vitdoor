...
test('user changes their password, corrects mismatched confirmation and sees success', async ({ page }) => {
  ...
  await expect(page.getByLabel('Nova senha', { exact: true })).toHaveAttribute('minlength', '6');
  ...
  await page.getByLabel('Confirmar nova senha').fill('123456');
  ...
  expect(sent).toEqual({ currentPassword: 'current-password-123', newPassword: '123456' });
...
test('server rejection preserves password form so the user can correct it', async ({ page }) => {
  ...
  await expect(page.getByLabel('Nova senha', { exact: true })).toHaveValue('123456');
